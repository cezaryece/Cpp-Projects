	...
	GbBase(parent), currProg(program), stopKlatkaName(""), totalTime(0), m_charTable(NULL)
{
	...
	else
	{
		...
		//kategoriaID
		foreach (int kategoriaID, currProg.katID)
		{
			foreach (QAction * action, actionsList)
			{
				if (action->data().toInt() == kategoriaID)
				{
...
void ProgramGroupBox::totalTimeChanged()
{
	...
	if (mw->aviTime <= 0)
	{
		mw->aviTime = (double) mw->vp.duration() / 1000.0;
...
bool ProgramGroupBox::save()
{
	...
	for (int i=0; i < katButton->menu()->actions().count(); ++i)
	{
		...
		if (action->isChecked())
			currProg.katID.append(action->data().toInt());
	}
	...
	QString katIDStr;
	foreach(int katID, currProg.katID)
		katIDStr.append(":" + QString::number(katID));
	katIDStr.remove(0,1);
