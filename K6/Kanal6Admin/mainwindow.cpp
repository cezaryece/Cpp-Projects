...
News MainWindow::getNewsFromDB(int newsID)
{
	...
	if (myQuery.exec("SELECT * FROM K6_archiwum WHERE id=" + QString::number(newsID)))
	{
		...
		inf.katID = myQuery.value(7).toString().split(":",QString::SkipEmptyParts);
...
void MainWindow::convertVideos()
{
	...
	if (cores > 1)
		threads = " -threads " + QString::number(cores);
	QString cmdLine = "ffmpeg -i \"" + fileName +"\" -vcodec libx264" + threads + " -deinterlace -b:v 300k -bufsize 1835k -s " + resize + " -acodec libvo_aacenc -b:a 64k -y -f mp4 -r 16 \""+ fpath + ".mp4\"";
	qDebug() << cores;
